...
module bikeKrankBolt(diameter = 19.5, height=3, pitch =1.25){
    center_hole = diameter/2-1.7;
    union(){
        difference(){
        RodStart(diameter, height, thread_len=12, thread_diam=diameter+1.5, thread_pitch=pitch);
        cylinder(40, center_hole,center_hole);
        }
        difference(){
        cylinder(3,29/2,29/2);
            translate([0,0,-0.5]){ //umbrako-hull
               cylinder(d=8.1/cos(30), h=5, $fn=6);
            }
         }
    }
}
//bikeKrankBolt(diameter = 19.5, height=3);
translate([0,0,0]){
        bikeKrankBolt(diameter = 20.0, height=3, pitch=1.25);
    }
